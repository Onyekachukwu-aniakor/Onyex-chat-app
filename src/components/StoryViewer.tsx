import { View, Text, Animated, Modal, Pressable, Image, TouchableOpacity } from 'react-native'
...
export default function StoryViewer({userStory, onClose}: Props) {
    ...
    const goNext = ()=>{
     ...
     if(currentIndex < userStory.stories.length - 1){
        ...
        /* it closes it if there is no next story */
        onClose()
     }
    }
  ...
  return (
    <Modal visible animationType='fade'  statusBarTranslucent>
        {/* statusBarTranslucent : Determines whether your modal should go under the system statusbar */}
        <View style={styles.container}>
            {/* progress bar */}
            <View style={styles.progressRow}>
               {userStory.stories.map((_, idx)=>(
                <View key={idx} style={styles.progressTrack}>
                   <Animated.View   style={[styles.progressFill, idx < currentIndex ? {width : '100%'} : idx === currentIndex ? {width : progressAnim.interpolate({
                    inputRange : [0,1],
                    outputRange : ['0%', '100%']
                   })} : {width : 0}]}/>
                </View>
            ...
            {/* media */}
            {story.mediaType  === 'video' ? (<StoryVideoPlayer uri={story.mediaUrl} style={styles.media}/>): (<Image source={{uri : story.mediaUrl}} style={styles.media} resizeMode='contain'/>)}
            {/* tap zones */}
