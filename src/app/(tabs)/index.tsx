    ...
    <SafeAreaView style={styles.safe } edges={['top']}>
     ...
     <View style={styles.header}>
      ...
      <View style={styles.searchRow}>
        <Ionicons name='search' size={22} color={Colors.outlineVariant}/>
        <TextInput  style={styles.searchInput} value={search} onChangeText={setSearch} placeholder='Search' placeholderTextColor={Colors.outlineVariant}/>
        {search.length > 0 && (
      ...
      {/* conversation list */}
      {loading ? (<ActivityIndicator style={{ marginTop : 40}}  color={Colors.primary}/>): (<FlatList data={filtered} keyExtractor={(c)=>c._id} contentContainerStyle={styles.listContent} renderItem={({item})=><ConvoItem  convo={item} selected={false} onPress={()=>openConvo(item)}/>}
      ListEmptyComponent={
        <View  style={styles.empty}>
         <Ionicons name='chatbubble-outline' size={44} color={Colors.outlineVariant}/>
         <Text style={styles.emptyTitle}>No conversations yet</Text>
         <Text style={styles.emptySubtitle}>Go to chat to start chatting</Text>
        </View>
      }/>
      )}
      <Text> </Text>
